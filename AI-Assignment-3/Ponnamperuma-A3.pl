...
%% Due Date: Thursday April 21st, 11:59PM

%% TYPE YOUR NAME/ID HERE: Kanishka Ponnamperuma, ID: - CO26615
...
delete(X, [X|Y], Y). % base case
delete(X, [Y, W|V], [Y|Z]) :- delete(X, [W|V], Z). % recursion
...
list_product(X, Y) :- (X = [], Y = _,fail; X = [A], Y = A). % base case
list_product([H|T], Product) :- list_product(T, Result), Product is H * Result. % recursion
...
last_element([X], X). % base case
last_element([_|T],Y):- last_element(T,Y). % recursion
...
list_length([_|T],N):- list_length(T,N1), N is N1+1. % recursion
